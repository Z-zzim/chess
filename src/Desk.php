    ...
    public function __construct() {
     ...
     public function move($move) {
        ...
        if (isset($this->figures[$xFrom][$yFrom])) {
           ...
           $color = $this->figures[$xFrom][$yFrom]->getColor(); // false - белые, true - черные 

            /* Определяем занято/свободно поле и свой/чужой */
           if (isset($this->figures[$xTo][$yTo])) {
               $DistCell = ($color == $this->figures[$xTo][$yTo]->getColor()) ? 1 : 2; // 1 - свой, 2 - чужой
           } else {
               $DistCell = 0; // свободная клетка
           }

            if ($counter > 0 AND $color == false) { // Если фигура белая-нечетная
                 if ($this->figures[$xFrom][$yFrom]->validation($xFrom, $yFrom, $xTo, $yTo, $DistCell, $this->figures))              
                     $this->figures[$xTo][$yTo] = $this->figures[$xFrom][$yFrom];
            }
            elseif ($counter == 0 AND $color == true) { // Если фигура черная-четная
                 if ($this->figures[$xFrom][$yFrom]->validation($xFrom, $yFrom, $xTo, $yTo, $DistCell, $this->figures))           
                     $this->figures[$xTo][$yTo] = $this->figures[$xFrom][$yFrom];
            } else {
                throw new \Exception("White black take turns");  
            }
        }
